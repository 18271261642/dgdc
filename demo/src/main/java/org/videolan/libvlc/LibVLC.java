...
public class LibVLC extends VLCObject<LibVLC.Event> {
    ...
    public void runVLCCommand(Context context, String... option) {
        if (null == option || option.length <= 0) {
            return;
        }

        runVLCCommand(context, Arrays.asList(option));
    }

    public void runVLCCommand(Context context, List<String> options) {
