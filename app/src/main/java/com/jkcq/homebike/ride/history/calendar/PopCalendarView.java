package com.jkcq.homebike.ride.history.calendar;
...
public class PopCalendarView extends ImageView {
    ...
    private static final String TAG = PopCalendarView.class.getSimpleName();
    ...
    public PopCalendarView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PopCalendarView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        this.context = context;
        drawablePad = DateUtil.dip2px(drawablePad);
        drawablePadTop = DateUtil.dip2px(drawablePadTop);
        ...
        for (int week = 0; week < mCells.length; week++) {
            for (int day = 0; day < mCells[week].length; day++) {
                if (tmp[week][day].whichMonth == CURRENT_MOUNT) { // 此月 开始设置cell
                    ...
                    mCells[week][day].setDateStr(mHelper.getYear() + "-" + formatTwoStr(mHelper.getMonth() + 1) + "-" + formatTwoStr(tmp[week][day].day));
...
//        }
        ...
        if (monthDataListen != null) {
            String strMonth = mRightNow.get(Calendar.YEAR) + formatTwoStr(mRightNow.get(Calendar.MONTH));
    ...
    public void goCurrentMonth() {
        ...
        requestLayout();
        if (monthDataListen != null) {
            String strMonth = mHelper.getYear() + formatTwoStr(mHelper.getMonth());
            monthDataListen.getMontData(strMonth);
        }
            ...
            for (Cell[] week : mCells) {
                for (Cell day : week) {
                    if (day.hitTest((int) event.getX(), (int) event.getY()) && event.getAction() == MotionEvent.ACTION_DOWN) {
                        ...
                        mToday = day;
                        long time = (day.getStartTime() - 6 * 3600) * 1000l;
        ...
        for (Cell[] week : mCells) {
            for (Cell day : week) {
                ...
                if (day.getStartTime() > 0) {
                    ...
                    canvas.drawCircle(day.getBound().centerX(), day.getBound().centerY(), CELL_WIDTH / 2.5f, day
                            .mCirclePaint);
                }
                if (day == mToday) {
                    ...
                    day.mCirclePaint.setColor(color);
                    //day.mPaint.setColor(getResources().getColor(R.color.bg_date_report_item));
                    day.mCirclePaint.setStrokeWidth(DateUtil.dip2px(1));
                    canvas.drawCircle(day.getBound().centerX(), day.getBound().centerY(), CELL_WIDTH / 2.5f, day
                            .mCirclePaint);
                }
                if (day == mToday) {
...
//                2019-01-09 17:02:00, 2019-01-08 17:01:00, 2019-01-07 17:09:00, 2019-01-02 17:01:00
    ...
    public void setSummary(ArrayList<String> strings) {
        List<Summary> list = new ArrayList<>();
        for (int i = 0; i < strings.size(); i++) {
            String dateStr = strings.get(i);
            String[] split = dateStr.split(" ");
