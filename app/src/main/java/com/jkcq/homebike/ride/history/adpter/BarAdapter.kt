...
class BarAdapter(data: MutableList<BarInfo>, count: Int) :
    ...
    override fun convert(holder: BaseViewHolder, item: BarInfo) {
        ...
        if (TextUtils.isEmpty(item.date)) {
            ...
            var toalWith = DateUtil.getScreenWidth(context) - DateUtil.dip2px(45f);
        ...
        if (item.isSelect) {
            ...
            holder.setTextColor(R.id.tv_date, context.resources.getColor(R.color.common_text_color))
        } else {
            /*  holder.setBackgroundResource(
                  R.id.view_current_bg,
                  R.drawable.shape_current_value_bar_nor_bg
              )*/
            holder.setVisible(R.id.tv_cal, false)
            holder.setVisible(R.id.tv_select, false)
            holder.setTextColor(R.id.tv_date, context.resources.getColor(R.color.common_tips_color))
        }
