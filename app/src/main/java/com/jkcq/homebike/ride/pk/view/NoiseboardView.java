package com.jkcq.homebike.ride.pk.view;
...
public class NoiseboardView extends View {
    ...
    private float mRealTimeValue = 0f; // 实时值

    private int mBigScaleRadius; // 大刻度半径
    private int mTargetScaleRadius; // 大刻度半径
    ...
    public NoiseboardView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mRadius = a.getDimensionPixelSize(R.styleable.NoiseboardView_noiseradius, dpToPx(80));
        ...
        mScaleTextSize = a.getDimensionPixelSize(R.styleable.NoiseboardView_noisescaleTextSize, spToPx(12));
        mUnitText = a.getString(R.styleable.NoiseboardView_noiseunitText);
        mUnitTextSize = a.getDimensionPixelSize(R.styleable.NoiseboardView_noiseunitTextSize, spToPx(14));
    ...
    private void init() {
        ...
        mSmallScaleRadius = mRadius - dpToPx(5);
        mBigScaleRadius = mRadius - dpToPx(10);
        mTargetScaleRadius = mRadius - dpToPx(20);
        ...
        mPaintScaleText.setStyle(Paint.Style.FILL);

        mPaintCirclePointer = new Paint();
        mPaintCirclePointer.setAntiAlias(true);
        mPaintCirclePointer.setStrokeCap(Paint.Cap.ROUND);
        ...
        mPaintRibbon.setColor(getResources().getColor(R.color.black_50));
    ...
    public void setTargetSpeed(int currentSpeed) {
        this.curSpeed = currentSpeed;
        curSpeed2 = curSpeed - 2;
        curSpeed3 = curSpeed + 2;
    }
    ...
    protected void onDraw(Canvas canvas) {
        // 绘制色带
        canvas.drawArc(mRectRibbon, 120F, 300F, false, mPaintRibbon);
        Double value = 270.0 / 200;
        mPaintScale.setStrokeWidth(dpToPx(2));
        mPaintScale.setColor(getResources().getColor(R.color.white));
        for (int i = 0; i <= mBigSliceCount; i++) {
            //绘制大刻度
            float angle = i * mBigScaleAngle + mStartAngle;
            point1 = getCoordinatePoint(mRadius, angle);
            point2 = getCoordinatePoint(mBigScaleRadius, angle);
        ...
        for (int i = 0; i < mSmallScaleCount; i++) {
            if (i % mScaleCountInOneBigScale != 0) {
                ...
                point1 = getCoordinatePoint(mRadius, angle);
                point2 = getCoordinatePoint(mSmallScaleRadius, angle);
        ...
        if (curSpeed != -1) {
            //绘制目标三角形
            mPaintCirclePointer.setColor(getResources().getColor(R.color.common_view_color));//#1DCE74
            float pointerDegree = (float) (135 + value * curSpeed);
            point1 = getCoordinatePoint(mRadius, (float) (135 + value * curSpeed2));
            point2 = getCoordinatePoint(mTargetScaleRadius, pointerDegree);
            point3 = getCoordinatePoint(mRadius, (float) (135 + value * curSpeed3));
            path.reset();
        ...
        //绘制中心点的圆
        mPaintCirclePointer.setColor(getResources().getColor(R.color.black_50));//#1DCE74
        mPaintCirclePointer.setStyle(Paint.Style.FILL);
        mPaintCirclePointer.setStrokeWidth(dpToPx(4));
        canvas.drawCircle(mCenterX, mCenterY, dpToPx(15), mPaintCirclePointer);
        ...
        path.reset();
        mPaintCirclePointer.setStyle(Paint.Style.FILL);
        point1 = getCoordinatePoint(mCircleRadius, initAngle + 90);
        path.moveTo(point1[0], point1[1]);
        // Log.e("绘制三角形指针", "point1[0]=" + point1[0] + ",point1[1]=" + point1[1] + ",initAngle=" + initAngle);

        point2 = getCoordinatePoint(mCircleRadius, initAngle - 90);
        path.lineTo(point2[0], point2[1]);
        // Log.e("绘制三角形指针", "point2[0]=" + point2[0] + ",point2[1]=" + point2[1]);
        point3 = getCoordinatePoint(mPointerRadius, initAngle);
        path.lineTo(point3[0], point3[1]);
        //Log.e("绘制三角形指针", "point3[0]=" + point3[0] + ",point3[1]=" + point3[1]);
        path.close();
        canvas.drawPath(path, mPaintCirclePointer);

        // 绘制三角形指针底部的圆弧效果

        canvas.drawCircle((point1[0] + point2[0]) / 2, (point1[1] + point2[1]) / 2, mCircleRadius, mPaintCirclePointer);
        ...
        mPaintCirclePointer.setStrokeWidth(10);

        float pointerDegree = (float) (135 + value * mRealTimeValue);
        Log.e("pointerDegree", "pointerDegree=" + pointerDegree);
        path.rewind(); // 清除直线数据，保留数据结构，方便快速重用
        path.arcTo(rectF, 130F, pointerDegree - 130);// 多截取一点弧会好看点
